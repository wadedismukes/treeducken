...
This vignette summarizes the workflow for simulating a host and symbiont tree using the `sim_cophyloBD` function. 
...
This model has a number of parameters that are input into the `sim_cophyloBD` function. Let us assume that we just want to simulate to a time of 2.0 units. We can then use `ave_tips_st` and `ave_tips_lt` to figure out the average number of extant tips that will be on our host and symbiont trees respectively.
...
We will now use the main function, `sim_cophyloBD` with the parameters we set in the previous section.
