...
This vignette summarizes the workflow for simulating a host and symbiont tree using the `sim_cbdp` function. 
...
This model has a number of parameters that are input into the `sim_cbdp` function. Let us assume that we just want to simulate to a time of 2.0 units. We can then use `ave_tips_st` and `ave_tips_lt` to figure out the average number of extant tips that will be on our host and symbiont trees respectively.
...
We will now use the main function, `sim_cbdp` with the parameters we set in the previous section.
...
```{r}
host_tree_locus_trees <- sim_lt_bdp(host_tree,
                                    gbr = 0.4,
                                    gdr = 0.2,
                                    lgtr = 0.0,
                                    num_loci = 10)
...
```{r}
symb_tree_locus_trees <- sim_lt_bdp(symb_tree,
                                    gbr = 0.2,
                                    gdr = 0.1,
                                    lgtr = 0.1,
                                    num_loci = 10)
