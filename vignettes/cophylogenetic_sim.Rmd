...
# that looks about right, let's assume there are no host speciations that are not cospeciations 
h_lambda <- 0.0
h_mu <- 0.3
c_lambda <- 1.0
# now we need to worry about the symbiont tree since only cospeciations are occurring
# we can use the same math as for the locus tree simulator
exp_tips_symb <- calculate_expected_leaves_locustree(t = 2.0, dup_rate = 0.2, loss_rate = 0.1, 8)
# a little less than 10 symbiont tips on average
s_lambda <- 0.2
s_mu <- 0.1
# let's assume that when symbionts speciate that they always inherit their ancestor's 
# host repertoire
s_her <- 0.0
```
...
host_symb_sets <- sim_cophylo_bdp(hbr = h_lambda,
                ...
                time_to_sim = 2.0,
                numbsim = 10)
print.multiCophylo(host_symb_sets, details = TRUE)
```
...
print.cophylo(tree_set_of_interest)
```
Notice that the output of `sim_cophylo_bdp` is a list of `cophylo` objects.
Each `cophylo` object is composed of 4 elements: `host_tree`, `symb_tree`, `association_mat`, an `event_history`.
...
parafit_test(host_tr = host_tree, symb_tr = symb_tree, assoc_mat = A, D = d, reps = 99)
```
...
```{r}
# of course that bit is a little too verbose, and we may be interested in a lot of trees
...
```{r}
host_tree_locus_trees <- sim_locustree_bdp(host_tree, gbr = 0.2, gdr = 0.1, lgtr = 0.1, num_loci = 10)
str(host_tree_locus_trees[[1]])
```
Now we will use the symbiont tree:

```{r}
symb_tree_locus_trees <- sim_locustree_bdp(symb_tree, gbr = 0.2, gdr = 0.1, lgtr = 0.1, num_loci = 10)
str(symb_tree_locus_trees[[1]])
```

Another option is simulating the locus tree and gene tree simultaneously.
This is output as a list with two elements: the locus tree and the set of gene trees.

```{r}
loctr_genetr_set <- sim_locustree_genetree_mlc(symb_tree,
                                               gbr = 0.2,
                                               gdr = 0.1,
                                               lgtr = 0.1,
                                               num_loci = 10,
                                               num_sampled_individuals = 1,
                                               num_genes_per_locus = 10,
                                               theta = 1)
```
...
```{r}
genetree_summary_stat(loctr_genetr_set, 1)
```

## Load in other datasets

You can also combine a host tree, symbiont tree, and association matrix into a `cophylo` object that can be input into functions in `treeducken`. The example below reads in the classic gopher and lice dataset from Hafner et al. 1994.
...
These are all then converted into `treeducken`'s `cophylo` class. 
You can then print out a summary of these and then calculate summary statistics on these. 
Currently, this only calculates the parafit statistic and conducts the permutation test for that statistic if the `event_history` element of `cophylo` is not set. 
If that is set it will count the numbers of each event.

```{r}
gopher_lice_map <- read.table(system.file("extdata", "gopher_lice_mapping.txt", package = "treeducken"), stringsAsFactors = FALSE, header = TRUE)

gopher_lice_assoc_matrix <- convert_assoc_table_to_matrix(gopher_lice_map)
gopher_tree <- ape::read.nexus(system.file("extdata", "gophers_bd.tre", package = "treeducken"))
lice_tree <- ape::read.nexus(system.file("extdata", "lice_bd.tre", package = "treeducken"))
gopher_lice_cophylo <- convert_to_cophylo(hostTree = gopher_tree, symbTree = lice_tree, assocMat = gopher_lice_assoc_matrix)
print(gopher_lice_cophylo)
cophylo_summary_stat(gopher_lice_cophylo)
plot.cophylo(gopher_lice_cophylo)
```
