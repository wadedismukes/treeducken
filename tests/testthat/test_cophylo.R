...
test_that("host_limit is set correctly", {
    expect_equal(get_number_hosts(sim_cophyloBD(hbr = 0.5,
                                                  hdr = 0.3,
                                                  sbr = 1.0,
                                                  sdr = 0.15,
                                                  host_exp_rate = 0.15,
...
test_that("sim_cophyloBD produces the right length trees", {
    expect_equal(get_all_host_tree_lengths(sim_cophyloBD(hbr = 0.5,
                                        hdr = 0.3,
                                        sbr = 1.0,
                                        sdr = 0.3,
                                        host_exp_rate = 0.0,
...
test_that("sim_cophyloBD produces the right number of trees", {
    expect_equal(length(sim_cophyloBD(hbr = 0.5,
                                        hdr = 0.3,
                                        sbr = 1.0,
                                        sdr = 0.3,
                                        host_exp_rate = 0.0,
...
test_that("sim_cophyloBD produces identical symbiont and host trees when only cospeciation is present", {
    expect_true(is_host_and_symbiont_the_same(t = 1.5, n = 10))
})


are_trees_identical_matrix_not <- function(t, n, disp_rate, ext_rate) {
    pairs <- sim_cophyloBD_ana(hbr = 0.0,
                            hdr = 0.0,
                            sbr = 0.0,
                            sdr = 0.0,
